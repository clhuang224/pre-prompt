...
PrePrompt 是一個為個人工作流程設計的小工具，重點不只是文字取代，而是維護一套可重用的敏感詞對照表，方便在把文章貼給 LLM 之前，先用自訂詞組替換掉敏感資料。
...
如果你想在本機測試，也可以直接用瀏覽器打開 `index.html`。
...
第一次進入頁面時，會自動帶入一組範例文章和詞組，方便快速理解功能。

1. 在「原文」輸入框貼上文章，或直接使用預設範例。
2. 在「詞組管理」區編輯規則，填入「原字詞」與「新字詞」。
3. 使用左側的 switch 啟用或停用規則，點選「刪除」可移除不需要的字詞。
4. 點選「新增」可增加空白規則。
